...
def different_proposals_and_K(file: str = "data/primates.phy", epochs: int = 200):
    for K in NO_LOOKAHEAD_K_VALS:
        # fmt: off
        args = dict(epochs=epochs, K=K, D=2, jc69=True, sample_branches=True, lookahead_merge=False, checkpoint_grads=False)
        # fmt: on
    ...
    for K in LOOKAHEAD_K_VALS:
        # fmt: off
        args = dict(epochs=epochs, K=K, D=2, jc69=True, sample_branches=True, lookahead_merge=True, checkpoint_grads=True)
        # fmt: on
