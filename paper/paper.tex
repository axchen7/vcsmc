...
% TODO replace this with Hyperbolic SMC
\begin{algorithm}
  \caption{Hyperbolic Sequential Monte Carlo}
  \label{alg:hyperbolic-smc}
  \begin{algorithmic}[1]
    \Require $\mathbf{Y} = \{Y_1, \cdots, Y_M\} \in \Omega^{N \times M}$, $\theta = (Q, \{\lambda_i\}_{i=1}^{|E|})$
    \State \textbf{Initialization.} $\forall k, s_0^k \leftarrow \bot, w_0^k \leftarrow 1/K$
    \For{$r = 1$ to $R = N - 1$}
    \For{$k = 1$ to $K$}
    \State \textbf{Resample} $\mathbb{P}(a_{r-1}^k = i) = \frac{w_{r-1}^i}{\sum_{l=1}^K w_{r-1}^l}$
    \For{$i = 1$ to $L = \binom{N-r}{2}$}
    \For{$m = 1$ to $M$}
    \State \textbf{Form look-ahead partial state}
    \[
      s_r^{k,m}[i] \sim q(\cdot|s_{r-1}^{a_{r-1}^k})
    \]
    \State \textbf{Compute potentials}
    \[
      w_r^{k,m}[i] = \frac{\pi(s_r^{k,m}[i]) \cdot \nu^{-}(s_{r-1}^{a_{r-1}^k})}{\pi(s_{r-1}^{a_{r-1}^k}) q(s_r^{k,m}[i] | s_{r-1}^{a_{r-1}^k})}
    \]
    \EndFor
    \EndFor
    \State \textbf{Extend partial state}
    \[
      s_r^k = s_r^{k,J}[I], \quad \mathbb{P}(I = i, J = j) = \frac{w_r^{k,j}[i]}{\sum_{l=1}^L \sum_{m=1}^M w_r^{k,m}[l]}
    \]
    \State \textbf{Compute weights}
    \[
      w_r^k = \frac{1}{ML} \sum_{i=1}^L \sum_{m=1}^M w_r^{k,m}[i]
    \]
    \EndFor
    \EndFor
    \Ensure $s_{1:R}^{1:K}, w_{1:R}^{1:K}$
  \end{algorithmic}
\end{algorithm}
