...
% TODO explain why hyperbolic space is a good choice for tree embeddings


\subsection{Embedding biological sequences}


% TODO cite? (https://en.wikipedia.org/wiki/Poincaré_disk_model#Lines_and_distance)
...
% TODO cite (find that paper that projected onto a hypersphere)
Experimentally, this technique yields better results than simply projecting onto a hypersphere of learnable radius.


% TODO mention that D=2 is sufficient, and the 2D Poincaré disk can be easily visualized
...
% TODO run an experiment to test different internal embedding functions
...
% TODO write up the math for the geodesic midpoint, which is simpler in the case that the two roots are already on the same hypersphere


\subsection{Sampling branch lengths}


% TODO test log normal vs "squared normal" again
...
% TODO cite https://www.stats.ox.ac.uk/~doucet/wang_bouchardcote_doucet_BayesianphylogeneticscombinatorialSMC_JASA2015.pdf
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \[
      q\qty(s_r^k | s_{r-1}^{i_\text{resample}}) = \mathbb{P}(t_\text{left} = i, t_\text{right} = j) \cdot f_\text{log-normal}(b_\text{left} | d_\text{left}, \sigma) \cdot f_\text{log-normal}(b_\text{right} | d_\text{right}, \sigma)
    \]
    \State Compute weight
    \[
      w_r^k = \frac{\pi\qty(s_r^k) \cdot \nu^-\qty(s_{r-1}^{i_\text{resample}})}{\pi\qty(s_{r-1}^{i_\text{resample}}) \cdot  q\qty(s_r^k | s_{r-1}^{i_\text{resample}})}
    \]
    \State
...
Hyperbolic SMC converges especially fast when the variance in the likelihood estimate is minimized. A large share of the variance is due to branch length sampling. One can imagine an alternate Hyperbolic SMC proposal where parent-child embedding distance are taken directly as branch lengths, with no sampling. We must drop the continuous $f_\text{log-normal}(b_\text{left/right} | d_\text{left/right}, \sigma)$ terms from $q(s_r^k | s_{r-1}^{i_\text{resample}})$, but this introduces a bias in the likelihood estimate. To see this effect more clearly, consider the expectation (over topology $\tau$ and branch lengths $\mathcal B$) of the log weights:


\begin{equation}
  \mathbb{E}_{\tau, \mathcal B \sim q\qty(\cdot | s_{r-1}^{i_\text{resample}})} \qty[\log w_r^k]
  = \dots - \mathbb{E}_{\tau, \mathcal B} \qty[\log q\qty(\tau | s_{r-1}^{i_\text{resample}})]  - \cancel{\mathbb{E}_{\tau, \mathcal B} \qty[\log q\qty(\mathcal B | \tau, s_{r-1}^{i_\text{resample}})]} \; .
\end{equation}

In the ``deterministic branch length'' setting, we drop the last term the penalizes the ELBO when the branch length distribution is sharply peaked. Namely, we overestimate the ELBO by $O(\log (1/\sigma))$, where $\sigma$ is is the interval over which sampling from $\text{Uniform}(b-\sigma, b+\sigma)$ approximates taking $b$ as the branch length.

In practice, we can form a biased likelihood estimator that---despite not being a valid ELBO---allows us to efficiently optimize the tip embeddings. In a two-phased training scheme, we first optimize embeddings while simultaneously exploring topology space under the low-variance, biased likelihood estimator. Empirically, the learned embeddings form a solid baseline for sampling under the true ELBO (i.e., with proper branch length distributions).
